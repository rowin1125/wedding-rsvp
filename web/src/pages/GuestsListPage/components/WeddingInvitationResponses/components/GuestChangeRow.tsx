...
type GuestChangeRowProps = {
    ...
    label: string;
    property:
        | keyof Omit<GetGuests['guests'][0], 'address'>
        | keyof NonNullable<
              GetWeddingInvitationResponses['weddingInvitationResponses'][0]['guestWeddingResponses'][0]
          >['guest'];
};

const GuestChangeRow = ({
    guestWeddingResponse,
    guest,
    property,
    label,
}: GuestChangeRowProps) => {
    const responseGuestRowData = guestWeddingResponse?.guest?.[property];
    const guestRowData = guest?.[property];
